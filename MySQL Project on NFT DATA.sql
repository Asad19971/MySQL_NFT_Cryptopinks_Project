...
OVER(ORDER BY event_date ROWS BETWEEN 50 PRECEDING AND CURRENT ROW)
FROM pricedata;
...
Select dayname(event_date) as Weekday, count(*) as NumberOfSales, Avg(eth_price)
From pricedata
Group by weekday
...
SELECT CONCAT(name, " was sold for $", ROUND(usd_price,-3), " to ", buyer_address, " from ", seller_address, " on ", event_date) FROM pricedata;
